...
<div class="container mt-4" id="dynamic-content">
    <div class="card mt-5">
        <div class="card-body">
            <h1 class="card-title">Edit Coaching Clinic</h1>
            <br>

            <form action="{{ route('coachingadmin.update', $coaching->id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="form-group">
                    <label for="nama">Nama</label>
                    <input type="text" class="form-control" id="nama" name="nama" value="{{ $coaching->nama }}" required>
                </div>

                <div class="form-group">
                    <label for="nip">NIP</label>
                    <input type="text" class="form-control" id="nip" name="nip" value="{{ $coaching->nip }}" required>
                </div>

                <div class="form-group">
                    <label for="tanggal">Tanggal Pengajuan</label>
                    <input type="date" class="form-control" id="tanggal" name="tanggal" value="{{ $coaching->tanggal }}" required disabled>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" id="tanggal_fix" name="tanggal_fix" value="{{ $coaching->tanggal_fix }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="jam_fix" name="jam_fix" required>
                        ...
                        <option value="{{ $jam->id }}" {{ $coaching->jams->contains($jam->id) ? 'selected' : '' }}>
                            {{ $jam->jam }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="media_teleconference" name="media_teleconference" required>
                        ...
                        <option value="{{ $zoom->media_teleconference }}" {{ $coaching->media == $zoom->media_teleconference ? 'selected' : '' }}>
                ...
                <div class="mt-3">
                    ...
                    <a href="{{ route('coachingadmin.index') }}" class="btn btn-secondary" id="btn-kembali">Kembali ke Daftar Formulir</a>
                </div>
            </form>
        </div>
    </div>
